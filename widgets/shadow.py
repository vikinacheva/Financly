...
class ShadowBox(ButtonBehavior, BoxLayout):
    ...
    def on_bcolor(self, *args, **kwargs):
        self._update_shadow()
    ...
    def _create_shadow(self, *args):
        ...
        t1 = self._create_boxshadow(ow, oh, radius, .5)
        ...
        w, h = ow + radius * 6.0, oh + radius * 6.0
        t2 = self._create_boxshadow(ow, oh, radius, .45)
    ...
    def _create_boxshadow(self, ow, oh, radius, alpha):
        ...
        x0, y0 = (w - ow) / 2., (h - oh) / 1.9
        x1, y1 = x0 + ow - 1, y0 + oh - 1

        bc = [92, 92, 92] if self.bcolor == [1,1,1,1] else self.bcolor[:3]
        draw.rounded_rectangle((x0, y0, x1, y1), self.radius[0], fill=(int(bc[0]), int(bc[1]), int(bc[2]), int(90 * alpha)))
        # rgba(64, 67, 108, 0.1)
